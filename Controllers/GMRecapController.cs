...
namespace SalesMetrics.Controllers
{
    public class GMRecapController : Controller
    {
        ...
        //        return View("RecapEntry", model); // Re-render with existing input
        //    }

        //    var userClaim = HttpContext.User.FindFirst("Users_ID");
        //    if (userClaim == null || !int.TryParse(userClaim.Value, out int user_id))
        //    {
        //        TempData["ErrorMessage"] = "Your session has expired or user ID is missing. Please log in again.";
        //        return RedirectToAction("Login", "Auth"); // or show error page
        //    }
        ...
        //    // Check and remove duplicate fields if already submitted this week
        //    var duplicateFields = await GetDuplicateFieldsAsync(model);
        //    if (duplicateFields.Any())
        //    {
        //        model.Fields = model.Fields
        //            .Where(f => !duplicateFields.Contains(f.FieldName, StringComparer.OrdinalIgnoreCase))
        //            .ToList();

        //        if (!model.Fields.Any())
        //        {
        //            TempData["ErrorMessage"] = "All submitted fields already exist with identical content. Nothing new was saved.";
        //            return View("RecapEntry", model);
        //        }

        //        if (duplicateFields.Any())
        //        {
        //            TempData["PartialSubmittedWarning"] = $"The following fields were already submitted (with identical content) and were skipped: {string.Join(", ", duplicateFields)}. You can edit the recap if you'd like to update them.";
        //        }

        //        TempData["PartialSubmittedWarning"] = $"The following fields were already submitted and were skipped: {string.Join(", ", duplicateFields)}. You can edit the existing recap to update them.";
        //    }
        ...
        //                insertEntryCmd.Parameters.AddWithValue("@LocationID", location);
        ...
        //            {
        //                tran.Rollback();
        //                TempData["ErrorMessage"] = "There was an error submitting your recap.";
        //                return View("RecapEntry", model);
        //            }
        //        }
        //    }
