            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var sql = @"
                SELECT
                    ...
                    C.CUM_CUSTOMER_NAME as CustomerName,
                    CAST(C.CUM_CUSTOMER_NUMBER AS VARCHAR(50)) as CustomerNumber,
                    S.SOH_ORDER_DATE as OrderDate,
                    S.SOH_DELIVERY_DATE as DeliveryDate,
                    CAST(S.SOH_TOTAL_AMOUNT AS DECIMAL(18,2)) as TotalAmount,
                    S.SOH_WHSMAS_ID as WarehouseId,
                    S.SOH_OPERATOR as Operator,
                    S.SOH_ORDERED_BY as OrderedBy,
                    CAST(CASE WHEN S.SOH_OPERATOR = 'Online' THEN 1 ELSE 0 END AS BIT) as IsOnlineOrder,
                    S.SOH_CANCELED_DATE as CanceledDate,
                    CAST(CASE WHEN S.SOH_CANCELED_DATE IS NOT NULL THEN 1 ELSE 0 END AS BIT) as IsCanceled,
